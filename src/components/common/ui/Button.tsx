...
const Button = forwardRef(
  <T extends React.ElementType>(
    ...
    // General Styles
    const generalStyles = [
      "whitespace-nowrap transition duration-200 border shadow-sm inline-flex items-center justify-center py-1.5 rounded-md font-semibold cursor-pointer", // Default
    ...
    // Sizes
    const defaultSize: Size = size ? size : "sm";

    const small = ["text-xs px-2"];
    const large = ["text-lg px-4"];
    ...
    const theme1 = "bg-theme-1 border-theme-1 text-white";

    const theme2 = "bg-theme-2 border-theme-2 text-white";
    ...
    return (
      <Component
        ...
        className={twMerge([
          generalStyles,
          defaultSize === "sm" && small,
          defaultSize === "lg" && large,
          ...
          variant === "dark" && dark,
          variant === "theme-1" && theme1,
          variant === "theme-2" && theme2,
          ...
          <>
            {loading ? (
              <LoadingIcon
                icon="tail-spin"
                color="#ffffff"
                className={twMerge(["mr-2", iconClasses])}
              />
            ) : (
              <AppIcon icon={icon} className={iconClasses} />
            )}
          </>
        )}
        {localizedLabel ? t(localizedLabel) : label}
          ...
          <>
            {loading ? (
              <LoadingIcon
                icon="tail-spin"
                color="#ffffff"
                className={twMerge(["ml-2", iconClasses])}
              />
            ) : (
              <AppIcon icon={icon} className={iconClasses} />
            )}
