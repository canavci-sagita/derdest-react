...
import { addEditCaseTypeAction } from "@/actions/definitions/case-types.actions";
import { useActionForm } from "@/lib/hooks/useActionForm";
import {
  addEditCaseTypeSchema,
  type AddEditCaseTypeDto,
...
interface AddEditCaseTypeModalProps {
  isOpen: boolean;
  onClose: (shouldRefetch?: boolean) => void;
  initialData?: AddEditCaseTypeDto | null;
}

const initialState = createFormState<AddEditCaseTypeDto>();

const AddEditCaseTypeModal = ({
  isOpen,
  onClose,
  initialData,
}: AddEditCaseTypeModalProps) => {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const localizedSchema = addEditCaseTypeSchema(t);
  ...
  return (
    <Modal
      title={t(isEditMode ? "editCaseType" : "addCaseType")}
      open={isOpen}
    ...
    >
      <form
        ...
        className="mt-8 grid-cols-1 gap-y-3"
      >
...
export default AddEditCaseTypeModal;
